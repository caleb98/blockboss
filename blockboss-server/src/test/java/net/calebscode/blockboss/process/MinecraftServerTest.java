...
public class MinecraftServerTest {
	...
	void sendInputs() throws IllegalArgumentException, IllegalAccessException, IOException, InterruptedException {
		...
		when(process.getStdout()).thenReturn(new BufferedReader(Reader.nullReader()));
		when(process.getStderr()).thenReturn(new BufferedReader(Reader.nullReader()));
	...
	void addStdoutListener()
		...
		var stringBuilder = new StringBuilder();
		var process = mockProcess(server);
		when(process.getStdin()).thenReturn(new BufferedWriter(Writer.nullWriter()));
	...
	private ProcessWrapper mockProcess(MinecraftServer server) throws IllegalArgumentException, IllegalAccessException {
		...
		Field processField = ReflectionUtils.findFields(MinecraftServer.class, f -> f.getName().equals("process"),
